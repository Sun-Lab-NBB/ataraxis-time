...
_TIMESTAMP_DTYPE: np.dtype[Any] = np.dtype("<u8")
"""The numpy dtype used to serialize and deserialize timestamps as little-endian unsigned 64-bit integers."""
...
def get_timestamp(
    ...
    Notes:
        The byte-array timestamp is encoded using the little-endian format.
    ...
    if output_format == TimestampFormats.BYTES:
        return convert_scalar_to_bytes(value=truncated_microseconds, dtype=_TIMESTAMP_DTYPE)
...
def convert_timestamp(
    ...
    if isinstance(timestamp, np.ndarray):
        ...
        microseconds = int(convert_bytes_to_scalar(data=timestamp, dtype=_TIMESTAMP_DTYPE))
    ...
    # NumPy bytes array format.
    if output_format == TimestampFormats.BYTES:
        return convert_scalar_to_bytes(value=microseconds, dtype=_TIMESTAMP_DTYPE)
...
def rate_to_interval(
    ...
    *,
    ...
    return convert_time(time=1.0 / rate_value, from_units="s", to_units=to_units, as_float=as_float)
...
def interval_to_rate(
    ...
    *,
    ...
    seconds = convert_time(time=interval_value, from_units=from_units, to_units="s", as_float=True)
...
def to_timedelta(
    ...
    """
    seconds = convert_time(time=time, from_units=from_units, to_units="s", as_float=True)
...
def from_timedelta(
    ...
    *,
    ...
    """
    return convert_time(time=timedelta_value.total_seconds(), from_units="s", to_units=to_units, as_float=as_float)
